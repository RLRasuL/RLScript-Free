...
//    CodeMirror/Monaco), wrapped in a <div class="not-prose">. RLScript's
...
const ZSProvider = (() => {
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…RLScript: result truncated to fit Arena's input limit - ` +
  ...
  // RLScript only supports DIRECT. Battle / Side by Side force a fresh A/B
  ...
  // bounces the tab), which RLScript can't drive. We nudge the mode dropdown
  ...
  function modeWarning() {
    ...
    if (um)
      return `Turn off <b>${um.label}</b> (${um.tip} in the composer) - ` +
        `RLScript only works in plain chat. ${um.label} mode uses a different ` +
    ...
    return `Switch the mode dropdown to <b>Direct</b> - RLScript only works in ` +
  ...
  // RLScript bar out of the way: the anchored bar is transparent but still a
  ...
  // up, the anchored RLScript bar (a real, full-width element hugging the
