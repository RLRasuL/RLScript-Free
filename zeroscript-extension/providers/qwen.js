...
const ZSProvider = (() => {
  ...
  function currentModelSupportsVision() {
    ...
    // During Qwen's initial render the selector can be temporarily absent. Do
    // not suppress the official viewport tool during that short window; if a
    // returned image cannot be processed, the core will surface that explicitly.
    if (!nm) return true;
    if (modelVis.has(nm)) return !!modelVis.get(nm);
    // Do not hide Roblox's screen_capture merely because Qwen changed its model
    // catalogue or the dropdown description was not mounted yet. Unknown Qwen
    // models are allowed optimistically; an explicit text-only/no-vision
    // description still changes the cache to false when the selector opens.
    return true;
  }
