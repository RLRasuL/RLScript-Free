...
(() => {
  ...
  // AI chat sites RLScript works on. Keep in sync with manifest.json
  ...
  async function waitForResponse(base) {
    ...
    while (Date.now() - lastActiveAt < TIMEOUT) {
      ...
      // settled in the DOM. Keep the active RLScript run alive by continuing
...
return result`;
  ...
  async function runTool(call) {
    ...
    // Virtual command: list the MCP server(s) RLScript is currently connected
    ...
    if (r.ok) {
      ...
      if (r.images && r.images.length) {
        // Show the capture in a left-hand RLScript popup (from the in-memory
  ...
  const decorate = {
    ...
    classify(item, next) {
      ...
      // ONLY in a real RLScript session (started or bootstrapping). Without
  ...
  const ui = (() => {
    ...
    function refreshSetup(bridgeConnected) {
      ...
      // conversation with no RLScript session (the "No agent here" state),
    ...
    // Left-hand RLScript popup showing the latest screen_capture. Fed from the
  ...
  // RLScript's own UI). A genuine "regenerate ↻" is always such a click;
  ...
  setInterval(() => {
    ...
    // The bare isHardGenerating() term is gated on a live RLScript session: on
  ...
  // A conversation IS a RLScript session if any rendered turn carries a
  // telltale artefact: the system-prompt marker, an injected tool-result /
  // system-note turn, or a RLScript command an assistant wrote. Works even
  ...
  function syncSessionState() {
    ...
    // via the SITE's own new-chat (not RLScript's button), the loop is bound to
  ...
  log(`RLScript content script ready (provider: ${P.id})`);
})();
