...
(() => {
  ...
  const AI_SITES = [
    { name: "DeepSeek", url: "https://chat.deepseek.com/", badge: "Recommended" },
    { name: "Gemini", url: "https://gemini.google.com/app" },
    { name: "Kimi", url: "https://www.kimi.com/" },
    { name: "GLM", url: "https://chat.z.ai/", badge: "Recommended" },
    { name: "Qwen", url: "https://chat.qwen.ai/", badge: "Recommended" },
  ...
  const A = {
    running: false,
    stop: false,
    // User-controlled safety gates. These are persisted by the menu and are
    // enforced in runTool as well as in the advertised command catalogue.
    allowAiTools: true,
    allowAiSkills: true,
  ...
  async function waitForResponse(base) {
    ...
    let nativeContinueAt = 0; // Arena's feedback panel may appear before its turn is readable
    ...
    while (Date.now() - lastActiveAt < TIMEOUT) {
      ...
      // Arena can show its task-feedback panel before the assistant turn has
      // settled in the DOM. Keep the active ZeroScript run alive by continuing
      // through that native panel directly instead of waiting for a response
      // classification that the panel itself prevents.
      if (P.id === "arena-agent" && Date.now() - nativeContinueAt > 1800 &&
          P.findContinueBtn && P.findContinueBtn() && P.clickContinueBtn && P.clickContinueBtn()) {
        nativeContinueAt = Date.now();
        diag("arena.nativeContinue");
        await sleep(350);
        continue;
      }
  ...
  const localVirtualTools = (server) => server === "roblox"
    ? enabledLocalVirtualTools().filter((tool) => !A.toolList.some((t) => bareToolName(t.name) === tool.name))
    : [];
  ...
  // This is deliberately a conservative, text-first check. Roblox's public MCP
  // catalogue exposes execute_luau and ScriptEditorService access, but not the
  // built-in Script Analysis window's lint/type diagnostics as structured data.
  // Compiling each source catches syntax errors for every AI, including models
  // that cannot see screen_capture images. The warning pass catches common
  // source-level diagnostics (including unknown globals) without pretending to
  // be a complete replacement for Studio's analyzer.
  function buildScriptAnalysisCode(scope) {
    const literal = JSON.stringify(scope || "game");
    return `local requestedScope = ${literal}
...
local root = resolvePath(requestedScope)
if not root then
  return {
    available = false,
    scope = requestedScope,
    error = "Scope not found: " .. requestedScope,
  }
end

local compiler = loadstring
local compilerAvailable = type(compiler) == "function"
...
local function maskLine(line, blockComment)
  local output = {}
  local i = 1
  while i <= #line do
  ...
  return table.concat(output), blockComment
end

local function bareWordTokens(code)
  local tokens = {}
  local cursor = 1
  local valid = true
  while true do
    local startAt, endAt = string.find(code, "[%a_][%w_]*", cursor)
    if not startAt then
      if string.match(string.sub(code, cursor), "^%s*$") == nil then
        valid = false
      end
      break
    end
    if string.match(string.sub(code, cursor, startAt - 1), "^%s*$") == nil then
      valid = false
    end
    table.insert(tokens, {
      name = string.sub(code, startAt, endAt),
      position = startAt,
    })
    cursor = endAt + 1
  end
...
local function scanIncompleteStatements(source, ignoredLine)
  local diagnostics = {}
  ...
  for line in (source .. "\\n"):gmatch("([^\\n]*)\\n") do
    ...
    local code
    code, blockComment = maskLine(line, blockComment)
    local tokens, bareStatement = bareWordTokens(code)
    local firstName = tokens[1] and tokens[1].name
    local controlOnly = firstName == "return" or firstName == "break" or firstName == "continue" or firstName == "else" or firstName == "elseif" or firstName == "end" or firstName == "until" or firstName == "do" or firstName == "then"
    if bareStatement and not controlOnly then
      local firstToken = lineNumber == ignoredLine and 2 or 1
      for tokenIndex = firstToken, #tokens do
...
local function scanWarnings(source)
  local known = {}
  local knownNames = {
    "_G", "_VERSION", "assert", "collectgarbage", "coroutine", "debug",
    "error", "getfenv", "ipairs", "loadstring", "next", "pairs", "pcall",
    "print", "rawequal", "rawget", "rawset", "select", "setfenv", "setmetatable",
    "string", "table", "task", "tonumber", "tostring", "type", "typeof", "unpack",
    "utf8", "xpcall", "math", "os", "bit32", "buffer", "game", "workspace", "script",
    "plugin", "shared", "Enum", "Instance", "CFrame", "Vector2", "Vector3", "Vector3int16",
    "Vector2int16", "UDim", "UDim2", "Color3", "BrickColor", "Ray", "Region3", "TweenInfo",
    "PhysicalProperties", "NumberRange", "NumberSequence", "ColorSequence", "DateTime", "Random",
    "Axes", "Faces", "Rect", "CatalogSearchParams", "OverlapParams", "RaycastParams",
  }
  ...
  for _, name in ipairs({
    ...
    "function", "if", "in", "local", "not", "or", "repeat", "return", "then", "true",
    "type", "until", "while",
  }) do
    keywords[name] = true
  end
  ...
  for lineNumber, line in ipairs(lines) do
    ...
    for name in code:gmatch("function%s+([%a_][%w_]*)") do
      known[name] = true
    end
    for name in code:gmatch("for%s+([%a_][%w_]*)") do
      known[name] = true
    end
    for name in code:gmatch("type%s+([%a_][%w_]*)") do
      known[name] = true
    end
    for parameters in code:gmatch("function[^%(]*%(([^%)]*)%)") do
  ...
  local warnings = {}
  local emitted = {}
  for lineNumber, code in ipairs(codeLines) do
    local _, bareStatement = bareWordTokens(code)
    for position, name in code:gmatch("()([%a_][%w_]*)") do
      if not keywords[name] and not known[name] then
        local previous = string.sub(code, position - 1, position - 1)
        local following = string.sub(code, position + #name, position + #name)
        local interesting = following == "(" or following == "." or following == ":"
        if not interesting then
          local prefix = string.sub(code, 1, position - 1)
          interesting = string.match(prefix, "[%s%(,%=:%+%-%*/%%%^<>~]$") ~= nil
          local before = string.match(prefix, "([%a_][%w_]*)%s*$")
          if before == "return" or before == "then" or before == "do" or before == "and" or before == "or" or before == "not" then
            interesting = true
          end
          local trimmed = string.match(code, "^%s*(.-)%s*$")
          if trimmed == name then
            interesting = true
          end
          if bareStatement then
            interesting = true
          end
        end
        ...
        if interesting and not emitted[name] then
          emitted[name] = true
          table.insert(warnings, {
            ...
            line = lineNumber,
            character = position,
            message = "Unknown global '" .. name .. "'; consider assigning to it first",
          })
        end
      end
    end
...
for _, scriptObject in ipairs(scripts) do
  ...
  if sourceOk and type(source) == "string" then
    ...
    if compilerAvailable then
      local compileOk, compiled, compileError = pcall(compiler, source, "=" .. scriptObject:GetFullName())
      if not compileOk then
        message = tostring(compiled)
      elseif not compiled then
        message = tostring(compileError or "syntax error")
      end
    end
    if message then
      compileLine = tonumber(string.match(message, ":(%d+):")) or tonumber(string.match(message, "(%d+),%d+"))
    end
    local syntaxDiagnostics = {}
    if message then
      table.insert(syntaxDiagnostics, {
        severity = "Error",
        path = scriptObject:GetFullName(),
        line = compileLine,
        message = message,
      })
    end
    for _, extra in ipairs(scanIncompleteStatements(source, compileLine)) do
      extra.path = scriptObject:GetFullName()
      extra.message = scriptObject:GetFullName() .. ":" .. extra.line .. ": " .. extra.message
      table.insert(syntaxDiagnostics, extra)
    end
    errorCount = errorCount + #syntaxDiagnostics
    for _, syntaxDiagnostic in ipairs(syntaxDiagnostics) do
      if #diagnostics < 100 then
        table.insert(diagnostics, syntaxDiagnostic)
      end
    end
    local warningDiagnostics = {}
    if not string.match(source, "^%s*%-%-!nocheck") then
      warningDiagnostics = scanWarnings(source)
    end
    warningCount = warningCount + #warningDiagnostics
    for _, warning in ipairs(warningDiagnostics) do
      if #diagnostics < 100 then
        warning.path = scriptObject:GetFullName()
...
return {
  available = true,
  mode = compilerAvailable and "syntax-plus-heuristic-warnings" or "heuristic-warnings-only",
  syntaxAvailable = compilerAvailable,
  scope = requestedScope,
  checked = checked,
  errors = errorCount,
  warnings = warningCount,
  diagnostics = diagnostics,
  truncated = errorCount + warningCount > #diagnostics,
  limitation = "Roblox's built-in Script Analysis lint and type diagnostics are not exposed as structured MCP data; warnings here are conservative text checks (including unknown globals), not a complete replacement for Studio's analyzer." .. (compilerAvailable and "" or " Syntax compilation is unavailable because this Studio execution context does not expose loadstring."),
}`;
  }
  ...
  async function runTool(call) {
    ...
    if ((name === "use_skill" || accessBareName === "skill") && !A.allowAiSkills) {
      return "ERROR: AI skills are disabled by the user. Do not call a skill again until the user enables AI skills in the ZeroScript menu.";
    }
    if (name !== "use_skill" && !A.allowAiTools) {
      return `ERROR: AI tools are disabled by the user. '${accessBareName || name}' was not run. Do not emit more ZeroScript commands until the user enables AI tools in the ZeroScript menu.`;
    }
    ...
    // Local skill loader: this does not spend a round trip through Studio and
    ...
    if (name === "use_skill") {
      ...
      if (!skill) {
        return `ERROR: unknown ZeroScript skill "${requested}". Available skills: ${Object.keys(ZS.BUILTIN_SKILLS || {}).join(", ") || "none"}.`;
      }
    ...
    // relabels the result so the model sees one coherent command.
    if (name === "script_analysis") {
      const hasExecute = A.toolList.some((tool) => bareToolName(tool.name) === "execute_luau");
      if (!hasExecute) return "ERROR: script_analysis requires Roblox's execute_luau command, which is not available right now.";
      const scope = String(args.scope || "game").trim().slice(0, 200) || "game";
      const nested = await runTool({
        tool: "execute_luau",
        arguments: { datamodel_type: "Edit", code: buildScriptAnalysisCode(scope) },
      });
    ...
    if (name === "list_commands" || name === "list_tools") {
      ...
      const scoped = [
        ...
        ...localVirtualTools(requested),
      ];
      if (!A.toolList.length && !scoped.length) return `Output of '${name}':\nNo commands available - the bridge or Roblox Studio may be offline.`;
      if (!scoped.length) {
      ...
      menuEl.innerHTML =
        `<div class="zs-menu-head"><span class="zs-menu-logo">ZeroScript</span><span class="zs-menu-tag">v${EXT_VERSION}</span></div>
         ...
         <section class="zs-menu-sec">
           ...
           <div class="zs-menu-note">Control whether ZeroScript lets the AI use connected commands or load skills. Changes are saved for future sessions.</div>
           ...
           <label class="zs-access-row"><input id="zs-allow-skills" type="checkbox" ${A.allowAiSkills ? "checked" : ""}><span class="zs-access-copy"><span class="zs-access-title">Allow AI skills</span><span class="zs-access-sub">ZeroScript and Roblox Studio skill workflows</span></span></label>
         </section>
         <section class="zs-menu-sec">
