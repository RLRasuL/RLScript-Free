...
- Added the text-first `script_analysis` command. It compiles Luau sources and
  returns structured syntax diagnostics plus conservative warnings for
  visionless AIs, while explicitly distinguishing that check from Roblox
  Studio's editor-only lint/type diagnostics.
...
- Added MiniMax Agent (agent.minimax.io), Grok (grok.com) and Claude
  (claude.ai) provider adapters, including composer injection, response
