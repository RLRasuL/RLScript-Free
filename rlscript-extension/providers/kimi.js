...
const RLProvider = (() => {
  ...
  const getEditor = () => {
    ...
    // barAnchor() then hugged the fixed RLScript bar over the editor. The real
  ...
  // its built-in tools over the RLScript command protocol. On K3 the only
  ...
  function nativeAgentModeOn() {
    ...
    // its own tooling instead of emitting RLScript command blocks.
    return !!(m && /swarm|agent/i.test(m.textContent || ""));
  }
  // Visible mode guard for the RLScript bar (core renderBar reads this every
  ...
  function modeWarning() {
    if (nativeAgentModeOn())
      return `Switch the model picker off <b>K3 Swarm</b> (pick <b>K3</b> or <b>Instantané</b>) - ` +
        `Kimi's own agentic mode replaces the RLScript commands with its native tools and breaks the agent loop.`;
    return "";
  }
  ...
  // Pick a model that actually speaks the RLScript command protocol, ONCE,
  ...
  return {
    ...
    // the capture is shown in the core's left-hand RLScript popup (ui.showImages).
    supportsVision: true,
    timings,
    ...
    // runner, etc.) instead of emitting the RLScript command blocks that drive
    ...
    unstableWarning:
      ...
      "If it stops acting in Roblox Studio and answers in plain text or runs its own tools, remind it to use the RLScript commands - or start a new session.",
