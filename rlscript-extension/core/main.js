...
return result`;
  ...
  async function runTool(call) {
    ...
    // script_analysis never computes diagnostics: the only source of truth is
    // Roblox Studio's Script Analysis window, read visually.
    if (name === "script_analysis") {
      String(args.scope || "game").trim().slice(0, 200);
      if (P.supportsVision) {
        return [
          "script_analysis does not compute diagnostics. The only source of truth is Roblox Studio's Script Analysis window, which shows the whole place (the scope parameter is ignored).",
          "Call screen_capture now and look at the image: if the Script Analysis window is visible, read every error and warning from it, then use script_read on each affected script and multi_edit for the smallest behavior-preserving fix. Repeat screen_capture until the window is clean.",
          "If the Script Analysis window is NOT visible in the capture, tell the user in one short sentence to open Window -> Script -> Analysis in Roblox Studio, then stop until they confirm it is open."
        ].join("\n");
      }
      return "script_analysis needs vision: the only source of truth is Roblox Studio's Script Analysis window and this AI cannot see images. Tell the user in one short sentence to open Window -> Script -> Analysis in Roblox Studio and read you the errors, then stop until they reply.";
    }
