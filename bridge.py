...
# never be edited/removed through the extension (it is what RLScript is FOR).
...
def _reclaim_bridge_port():
    ...
    if "bridge.py" not in cmdline.lower():
        log(f"port {PORT} is held by pid {pid_i} ('{name}') but it does not look "
            f"like a RLScript bridge - leaving it alone.", "yl")
        return False
    log(f"port {PORT} is held by a leftover RLScript bridge (pid {pid_i}) from a "
